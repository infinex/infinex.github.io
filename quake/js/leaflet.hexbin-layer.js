L.HexLayer = L.Class.extend({
    ...
    initialize: function (data, options) {
        ...
        this._rscale = d3.scale.sqrt().range([0, this.options.radius]).clamp(false);
        this._data = data;
        this._levels = {};
    },
    ...
    _createHexagons: function (container) {
        var layout = this._layout,
            data = this._data.features.map(function (d) {
                var coords = this._project(d.geometry.coordinates)
...
//                .attr("d",function (d) {return layout.hexagon(ref._rscale(d.length))});
                .attr("d",function (d) {return layout.hexagon()});
    },
